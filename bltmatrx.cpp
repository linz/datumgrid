...
BLT_Def::BLT_Def( int nRows ) : nRows(nRows ) {
    rows = new int[nRows];
    for( int i=0; i<nRows; i++ ) rows[i] = i+1;
    }
...
void BLT_Def::SetNonZero( int row, int col ) {
...
BLT_Matrix::BLT_Row::~BLT_Row(){ if( values ) delete [] values; }


// This implementation only allows the size to be set once...

void BLT_Matrix::BLT_Row::SetSize( int firstCol, int lastCol ) {
   assert( values == 0 );
   assert( last >= first );
...
//   int firstr = first > r2.first ? first : r2.first;
//   int lastr = last < r2.last ? list : r2.last;
...
BLT_Matrix::BLT_Matrix( int size ) {
   assert( size > 0 );
   nRows = size;
...
BLT_Matrix::BLT_Matrix( int size, int bandwidth ) {
   assert( size > 0 );
   assert( bandwidth > 0 );
   nRows = size;
   rows = new BLT_Row[size];
   for( int i = 1; i <= nRows; i++ ) {
       int col = i - bandwidth + 1;
       if( col < 1 ) col = 1;
       Row(i).SetSize( col, i );
       }
   status = Entering;
...
void BLT_Matrix::Zero() {
   for( int i = 1; i <= nRows; i++ ) Row(i).Zero();
   status = Entering;
   badrow = 0;
   }
...
int BLT_Matrix::FormCholeskiDecomposition(){
